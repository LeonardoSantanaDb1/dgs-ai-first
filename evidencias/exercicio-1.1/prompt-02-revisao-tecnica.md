...
Submeter a análise técnica produzida na primeira iteração a uma revisão crítica independente, buscando identificar riscos não considerados, premissas otimistas, fragilidades arquiteturais e oportunidades de melhoria antes da consolidação do entregável final.

## Prompt

Atue como um Arquiteto de IA Sênior especializado em sistemas RAG corporativos.

Revise criticamente a análise técnica abaixo.

Seu papel é identificar:

- Riscos técnicos não considerados.
- Estimativas excessivamente otimistas.
- Problemas na estratégia de chunking.
- Problemas na análise de contexto.
- Problemas na estratégia de retrieval.
- Recomendações arquiteturais adicionais.

Analise como se estivesse realizando uma revisão técnica para aprovação de arquitetura em ambiente corporativo.

Não assuma que a análise está correta.

Seu objetivo é encontrar falhas.

Questione premissas, identifique riscos ignorados e aponte pontos que poderiam causar problemas em produção.

Sempre que possível, explique por que determinado risco é relevante.

Apresente a resposta utilizando a seguinte estrutura:
