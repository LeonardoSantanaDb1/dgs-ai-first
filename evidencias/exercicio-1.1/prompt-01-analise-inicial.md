...
Obter uma avaliação técnica dos desafios envolvidos na construção de uma solução RAG corporativa para a NovaTech.
...
O contexto que o LLM recebe a cada pergunta é limitado pela janela de contexto do modelo.
...
- onde ficam posicionados no prompt (lost in the middle)
- o que mais está competindo por atenção (system prompt, histórico, instruções)

Atue como um Arquiteto de IA Sênior especializado em sistemas RAG corporativos.
Com base no cenário fornecido, realize uma análise técnica detalhada contemplando:
1. Para cada tipo de fonte documental:
   - desafios de ingestão
...
5. Recomendações arquiteturais para uma solução corporativa.

Apresente a resposta em formato estruturado, utilizando tabelas sempre que possível.

Considere um ambiente produtivo corporativo.

Não simplifique a análise.
